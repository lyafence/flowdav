...
func main() {
	...
	var multiBackend *storage.MultiBackend
	...
	if len(appCfg.WebDAV.Backends) > 0 {
		...
		multiBackend = storage.NewMultiBackend(backends)
		backend = multiBackend
		logger.Info("Using MultiBackend with %d providers", multiBackend.NumBackends())
	} else {
